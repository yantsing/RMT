...
% The helpers effectively confuse the eavesdropper compared with the approach of artificial noise generated by the transmitter.  This is because the helpers are usually not collocated with the transmitter.  The trade-off is the coordination requirement of the helpers and the transmitter.  More helpers improve the secure communication performance at a cost of more stringent coordination requirement. 
...
\begin{figure}[!htbp] 
	\centering
	\includegraphics[width=8.7cm]{channel.png} % requires the graphicx package
...
%
%\begin{eqnarray}
%\mathrm{SINR}_e &=& \frac{\left|\mathbf{r}^H \mathbf{G}_{0}^H\mathbf{w}_{0}\right|^2}{\sigma_m^2 + \sum_{i=1}^{N}\mathbf{g}_{i}^H\mathbf{Q}_{i}\mathbf{g}_i} \label{eq:secure_capacity}.
%\end{eqnarray}
%
%
%The channel capacity of Eve is 
%\begin{eqnarray}
%\log\left|\mathbf{I}+\mathbf{R}^{-1}\mathbf{G}_0^H\mathbf{w}_0\mathbf{w}_0^H\mathbf{G}_0\right|
%\end{eqnarray}
%
...
%
%
%\section{Robust Convex Optimization of Secure Communication}
...
%\end{eqnarray}
%We assume the channel mismatches lie in the bound sets $\mathcal{G}_i=\{\tilde{\mathbf{G}}_i:\text{Tr}\left(\tilde{\mathbf{G}}_i^H \mathbf{R}_i\tilde{\mathbf{G}}_i\right) \leq 1\}$ are the channels error vectors;the hermitian positive semi-definite  matrix $\mathbf{R}_i $ defines the variation of channel $\mathbf{G}_i$. 
%
%\section{Robust Convex Optimization of Secure Communication} \label{sec:robust programming}
%Considering ZF cooperative Jamming. 
...
%where 
%\begin{eqnarray}
%\mathbf{R} = \sigma_e^2\mathbf{I} +\sum_{k = 1}^{K}\mathbf{G}_i\mathbf{Q}\mathbf{G}^H 
%\end{eqnarray}
%
%\begin{eqnarray}
%dR_s = - \text{Tr}\left((\mathbf{R}^{-1}-(\mathbf{R}+\mathbf{G}_0\mathbf{Q}_0\mathbf{G}_0^H))\right)d\mathbf{R}
%\end{eqnarray}
%
%\begin{eqnarray}
%R_s \sim \log\left(1 + \frac{ \mathbf{h}_{0}^H\mathbf{Q}_{0}\mathbf{h}_{0}}{\sigma_e^2 + \sum_{i=1}^{N}\mathbf{h}_{i}^H\mathbf{Q}_{i}\mathbf{h}_i}\right) - \log
%\end{eqnarray}
...
%In general, maximizing $R_s$ over $\mathbf{Q}_0$ and $\mathbf{Q}_i$ is a non-convex problem. In this paper, we decouple the optimization process into two
%steps, in which $\mathbf{Q}_i$ are calculated first, then $\mathbf{Q}_0$ is calculated. 

%In \cite{huang_robust_2012}, there is only one helper and it is reasonable to let the helper to generate the maximum interference to the eavesdropper. 
...
%& \underset{\{\mathbf{Q}_i \succeq \mathbf{0}\}}{\text{minimize}\}}
%& & \sum_{k=1}^{K}\text{Tr}(\mathbf{Q}_i)\\
%& \text{subject to}
%& & \text{Tr}\left(\mathbf{G}_i\mathbf{Q}_i\mathbf{G}_i^H\right) \geq \gamma_i  \label{eq:artificial noise constraint},\\
%& & & \sum_{i = 1}^{N}\gamma_i = \Gamma,\\
%&&& \text{Tr}\left(\mathbf{Q}_i\right) \leq P_i, \forall i = 1,\cdots,N.
%\end{aligned}
%\end{eqnarray}
%The Lagrangian is
%\begin{eqnarray}
%\mathcal{L} = \sum_{k=1}^{K}\text{Tr}(\mathbf{Q}_i)+\lambda\left(\Gamma -\sum_{i=1}^{N}\min_{\{\mathbf{G}_i \in \mathcal{G}_i\}}\text{Tr}\left( \mathbf{G}_i\mathbf{Q}_i\mathbf{G}_i^H\right) \right)
%\end{eqnarray}
...
%\begin{aligned} \label{eq:numerator problem1}
%& \underset{\{\mathbf{Q}_i \succeq \mathbf{0}\}}{\text{minimize}\}}
%& & \sum_{k=1}^{K}\text{Tr}(\mathbf{Q}_i) - \lambda \min_{\mathbf{G}_i \in \mathcal{G}_i}\text{Tr}\left(\mathbf{G}_i\mathbf{Q}_i\mathbf{G}_i^H\right)
%\end{aligned}
%\end{eqnarray}
%with $\text{Tr}\left(\mathbf{Q}\right) \leq P_i$. The dual variable $\lambda$ can be updated using bisection method. The problem can be transformed to 
%\begin{eqnarray}
%\begin{aligned} \label{eq:numerator problem1}
%& \underset{\mathbf{Q}_i \succeq \mathbf{0},\gamma_i \geq 0}{\text{minimize}\}}
%& & \sum_{k=1}^{K}\text{Tr}(\mathbf{Q}_i) - \lambda \gamma_i\\
%& \text{subject to}
%& & \min_{\{\mathbf{G}_i \in \mathcal{G}_i\}}\text{Tr}\left(\mathbf{G}_i\mathbf{Q}_i\mathbf{G}_i^H\right) = \gamma_i  \label{eq:artificial noise constraint}
%\end{aligned}
%\end{eqnarray}
%
%\begin{eqnarray}
%&\underset{\mathbf{G}_i \in \mathcal{G}_i }\min\text{Tr}\left(\mathbf{G}_i\mathbf{Q}_i\mathbf{G}_i^H\right) \geq  \gamma_i, \forall i = 1,\ldots,N\label{eq:artificial noise constraint decomposed}\nonumber\\
%&\sum_{i =1}^{N}\gamma_i = \Gamma.
%\end{eqnarray}
%
%
...
%& & \sum_{k=1}^{K}\text{Tr}(\mathbf{Q}_i)\\
%& \text{subject to}
%& & \min_{\{\mathbf{G}_i \in \mathcal{G}_i\}}\text{Tr}\left(\mathbf{G}_i\mathbf{Q}_i\mathbf{G}_i^H\right) \geq \gamma_i  \label{eq:artificial noise constraint},\\0
%& & & \sum_{i = 1}^{N} \gamma_i \geq \Gamma
%\end{aligned}
%\end{eqnarray}
%
