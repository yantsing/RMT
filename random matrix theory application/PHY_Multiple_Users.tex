...
Fakoorian~\emph{et al.} study the MIMO wiretap channel and obtain the structure of the artificial noise covariance matrix of the helper that guarantees a secrecy rate larger or at least equal to the secrecy capacity of the wiretap channel with no jamming signal \cite{fakoorian_solutions_2011}. These methods assume that the transmitter and the helper have perfect CSI. Pei~\emph{et al.} investigate the case where the quantized channel information regarding the legitimate receiver is available at the transmitter and helper due to finite-rate feedback, but perfect CSI for the channel to the eavesdropper is assumed~\cite{pei_adaptive_2014}.
...
We assume that the transmitter and the helpers know the channels toward the legitimate receiver.  The channels toward the eavesdropper are not perfectly known.  The uncertainties of the eavesdropper's channels are described with an ellipsoid model.   Given a required value of the achievable secrecy rate, the transmit strategies of the helpers are optimized and a distributed algorithm is developed for transmission of a large number of helpers. 
...
Consider a network composed of several transmitters, a receiver and an eavesdropper. When one transmitter desires secure communication with the receiver, the other transmitters act as helpers to degrade the performance of the eavesdropper. The intended transmitter is called Alice; the intended receiver is called Bob; the eavesdropper is called Eve. A secure communication scenario is shown in Fig.~\ref{fig:system}. Alice wants to send messages to Bob. Without loss of generality, assume there is one eavesdropper. In order to implement secure communication, the $N$ helpers send artificial noise to degrade the receiving performance of Eve.  Alice and every helpers are equipped with $M$ antennas each. The information of $M$ dimensional transmission channel vectors from Alice to Bob $\mathbf{h}_0$, and from the helpers to Bob $\mathbf{h}_i, i = 1,\ldots,N$ are known.
\begin{figure}[!htbp]
	\centering
...
\begin{eqnarray}
y_k =\mathbf{h}_{0k}^{H}\mathbf{w}_k u_k + \sum_{j \neq k}^{K}\mathbf{h}_{0k}^{H}\mathbf{w}_k \sum_{i=1}^N \mathbf{h}_{ik}^H\mathbf{x}_{i} + n,
\end{eqnarray}
where $u_k$ is the message transmitted from BS to $k$th user, and $\mathbf{E}\{|u_k|^2\} = 1$; $\mathbf{E}\{\cdot\}$ means expectation; $[\cdot]^H$ denotes hermitian operation. $\mathbf{w}_{k}$  is the $M \times 1$ beamforming vector to  $k$th user, which satisfies the transmit power constraint $\|\mathbf{w}_k\|_2^2 \leq P_0$. $\mathbf{x}_i, i = 1,\ldots,N $ are the signals transmitted from the helpers. Signal $\mathbf{x}_i$ is $M$ dimensional  complex Gaussian random vector $\mathbf{x}_i \sim \mathcal{N}(\mathbf{0}, \mathbf{Q}_i)$, which needs to satisfy power constraint $\mathbf{E}\{\|\mathbf{x}_i\|_2^2\}  = \mathrm{Tr}(\mathbf{E}\{\mathbf{x}_i\mathbf{x}_i^H\} )= \mathrm{Tr}(\mathbf{Q}_i) \leq P_i$. Let the noise be $n \sim \mathcal{N}(0,\sigma_n^2)$. The signal-to-interference-plus-noise ratio of Bob is
\begin{eqnarray} \label{eq:SINR of Bob}
\mathrm{SINR}_k = \frac{\left| \mathbf{h}_{0k}^H\mathbf{w}_{k}\right|^2}{\sigma_n^2 + \sum_{i=1}^{N}\mathbf{h}_{i}^H\mathbf{Q}_{i}\mathbf{h}_i}.
\end{eqnarray}
...
The basic idea of the transmit strategy of the system is to let Alice maximize the signal-to-interference-plus-noise ratio of Bob, and leave the job of guaranteeing secure communication to the helpers by generating noise to Eve. From \eqref{eq:SINR of Bob} and \eqref{eq:ZF constraint} The signal-to-interference-plus-noise ratio of Bob is 
\begin{eqnarray}
...
& \underset{\{\mathbf{Q}_i \succeq \mathbf{0}\},\{\tau_i\}}{\text{minimize}}
& & \sum_{i = 1}^{N}\mathrm{Tr}(\mathbf{Q}_i) \\
& \text{subject to}
& & \mathrm{Tr}(\mathbf{Q}_i) \leq P_i, \forall i = 1, \ldots, N\\
&&& \mathbf{h}_i^H \mathbf{Q}_i \mathbf{h}_i= 0, \forall i = 1,\ldots,N\\
...
The term $\frac{\tau_i*}{\mathrm{Tr}(\mathbf{Q}_i^*)}$ defines the interference generated per unit power by Helper-$i$. We call it the transmit efficiency of Helper-$i$. Proposition 1 tells us the helpers with higher transmit efficiency should be exploited first until all the transmit power of the helpers is used.
...
Proposition 2 describes the fact that no matter how much interference is needed to be transmitted by each helper, the transmit efficiency remains the same.  Based on Proposition 1 and Proposition 2, we propose the following algorithm.
